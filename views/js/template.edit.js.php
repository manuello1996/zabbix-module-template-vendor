...
window.template_edit_popup = new class {
	...
	init({templateid, warnings, csrfTokens}) {
	...
	submit() {
		// Align CSRF token with the target action to avoid mismatched token errors.
		this.#setCsrfToken(this.templateid === null ? 'create' : 'update');
	...
	#setCsrfToken(action) {
		const csrf_input = this.form.querySelector('input[name="<?= CSRF_TOKEN_NAME ?>"]');

		if (!csrf_input) {
			return;
		}

		if (action === 'create' && this.csrfTokens && this.csrfTokens.create) {
			csrf_input.value = this.csrfTokens.create;
		}
		else if (action === 'update' && this.csrfTokens && this.csrfTokens.update) {
			csrf_input.value = this.csrfTokens.update;
		}
	}
