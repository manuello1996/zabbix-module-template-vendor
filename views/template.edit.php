...
$form = (new CForm())
	->addItem((new CVar(CSRF_TOKEN_NAME, CCsrfTokenHelper::get('template.update')))->removeId())
...
$template_tab->addItem([
	...
	new CFormField(
		// Render editable vendor/version fields in the popup when user has RW permissions.
		new CHorList([
			(new CTextBox('vendor_name', $data['vendor_name'], $data['vendor_readonly'],
				DB::getFieldLength('hosts', 'vendor_name')
			))
				->setWidth(ZBX_TEXTAREA_SMALL_WIDTH)
				->setAttribute('placeholder', _('Vendor')),
			(new CTextBox('vendor_version', $data['vendor_version'], $data['vendor_readonly'],
				DB::getFieldLength('hosts', 'vendor_version')
			))
				->setWidth(ZBX_TEXTAREA_SMALL_WIDTH)
				->setAttribute('placeholder', _('Version'))
		])
	)
]);
	...
	->addItem(
		(new CScriptTag('
			template_edit_popup.init('.json_encode([
				...
				'csrfTokens' => [
					'create' => CCsrfTokenHelper::get('template.create'),
					'update' => CCsrfTokenHelper::get('template.update')
				]
			], JSON_THROW_ON_ERROR).');
		'))->setOnDocumentReady()
	);
