...
class TemplateCreate extends CController {
	...
	protected function doAction(): void {
		try {
			...
			$vendor_name = trim($this->getInput('vendor_name', ''));
			$vendor_version = trim($this->getInput('vendor_version', ''));

			// Validate vendor/name pairing before API create for a clearer error message.
			if (($vendor_name === '') !== ($vendor_version === '')) {
				$this->setResponse(new CControllerResponseData(['main_block' => json_encode([
					'error' => [
						'title' => _('Cannot add template'),
						'messages' => [_('Vendor and version must both be filled in or both be empty.')]
					]
				], JSON_THROW_ON_ERROR)]));
